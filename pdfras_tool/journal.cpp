...
void journal::println(char *path, const int line, const char *function, const log_level level, const char *fmt, ...)
{
  ...
  if (!line_count++) {
	  if (level >= dbg) {
		  ...
		  println(__FILE__, __LINE__, __FUNCTION__, level, "%s log_level=%d (built %s at %s)", tbuf, lvl, __DATE__, __TIME__);
	  }
  }
  
  char tbuf[6];
  strftime(tbuf,sizeof(tbuf),"%H:%S",now);//if change format string, adjust sizeof tbuf decl on prior line
